...
void BaseSurface::blit(int x, int y, SDL_Surface* src)
{
    ...
    dst_rect.x = static_cast<Sint16>(x);
    dst_rect.y = static_cast<Sint16>(y);
    dst_rect.w = static_cast<Uint16>(data->w);
    dst_rect.h = static_cast<Uint16>(data->h);
...
void BaseSurface::blit(Sprite& sprite)
{
    ...
    src_rect.x = static_cast<Sint16>((sprite.currentFrame - 1) 
                                    * sprite.getWidth());
    src_rect.y = 0;
    src_rect.w = static_cast<Uint16>(sprite.getWidth());
    src_rect.h = static_cast<Uint16>(sprite.getHeight());

    blit((int)sprite.getX(), (int)sprite.getY(), sprite.data, src_rect);
}

void BaseSurface::setPix(int x, int y, Uint8 r, Uint8 g, Uint8 b)
{
...
//  else *((Uint16 *)data->pixels + y * data->pitch / 2 + x) = color;
    else *((Uint16*)data->pixels + ((y * data->pitch) >> 1) + x) = color;
}

void BaseSurface::setPix(int x, int y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
...
{
    Uint16 color = SDL_MapRGB(data->format, r, g, b);
#ifndef OPTIMIZE_SCREEN_WIDTH_640
//  *((Uint16 *)data->pixels + y * data->w + x) = color;
...
void BaseSurface::blendPix(int x, int y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
...
void BaseSurface::getPix(int x, int y, Uint8* r, Uint8* g, Uint8* b) const
{
    Uint32 pixel;
    if (data->format->BitsPerPixel > 16)
//      pixel = *((Uint32*)data->pixels + y * data->pitch / 4 + x);
        pixel = *((Uint32*)data->pixels + ((y * data->pitch) >> 2) + x);
//  else pixel = *((Uint16*)data->pixels + y * data->pitch / 2 + x);
    else pixel = *((Uint16*)data->pixels + ((y * data->pitch) >> 1) + x);
    SDL_GetRGB(pixel, data->format, r, g, b);
}

void BaseSurface::getPix(int x, int y, Uint8* r, Uint8* g, Uint8* b, Uint8* a) const
{
...
{
#ifndef OPTIMIZE_SCREEN_WIDTH_640
//  Uint16 pixel = *((Uint16*)data->pixels + y * data->w + x);
...
// compact.
void BaseSurface::line(int x1, int y1, int x2, int y2, Uint8 r, Uint8 g, Uint8 b)
{
...
// This implementation seems far too verbose. Need to make this more
// compact.
void BaseSurface::lineAA(int x1, int y1, int x2, int y2, Uint8 r, Uint8 g, Uint8 b)
{
    int xPoint1, yPoint1, xPoint2, yPoint2;
    int dx, dy;
    int rValue;
    ...
    if (m >= 0 && m < 1) {
        ...
        alpha = (Uint8)(((float)rValue / (float)dx) * 255);
    ...
    if (m >= 1) {
        ...
        alpha = (Uint8)(((float)rValue / (float)dy) * 255);
        blendPix(xPoint1, yPoint1, r, g, b, alpha);
...
void BaseSurface::fillRect(int x, int y, int w, int h, Uint8 r, Uint8 g, Uint8 b)
{
    SDL_Rect tmp;

    tmp.x = static_cast<Sint16>(x);
    tmp.y = static_cast<Sint16>(y);
    tmp.w = static_cast<Uint16>(w);
    tmp.h = static_cast<Uint16>(h);
...
void BaseSurface::print(int x, int y, Uint32 value, Uint8 r, Uint8 g, Uint8 b)
{
...
void BaseSurface::drawChar(int x, int y, char c, Uint32 color)
{
    ...
    int ix, iy, k;
    Uint8* charpos;
    Uint8 bits[8] = { 128, 64, 32, 16, 8, 4, 2, 1 };
    Uint8* bitpos;
    Uint8* curpos;
    int xTmp, yTmp;
        ...
        for (ix = 0; ix < 8; ix++) {
            if ((*charpos & *bitpos) == *bitpos) {
                if (data->format->BitsPerPixel > 16)
                    ...
                    *((Uint16*)data->pixels + yTmp * data->pitch / 2 + xTmp) 
                        = color;
            }
